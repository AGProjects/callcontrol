...
from callcontrol.sip import sipClientInfo
from callcontrol.datatypes import Structure
...
class Call(Structure):
    ...
    def __expire(self):
        self.expired = True ## only the Timer thread can set this flag, so don't protect with locking
        sip = SipClient()
    ...
    def end(self):
        ...
        try:
            try:
                del ControlledCalls[self.callid]
            except KeyError:
